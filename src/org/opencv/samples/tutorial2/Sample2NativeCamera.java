...
public class Sample2NativeCamera extends Activity implements CvCameraViewListener {
    ...
    public Mat onCameraFrame(Mat inputFrame) {
        switch (Sample2NativeCamera.viewMode) {
            ...
            {
                Imgproc.Canny(inputFrame, mIntermediateMat, 70, 100);
                for (int i=0;i<mIntermediateMat.rows();i++)
                	for (int j=0;j<mIntermediateMat.cols();j++)
                	{
                		double[] elements = mIntermediateMat.get(i, j);
                		for (int k=0;k<elements.length;k++)
                			elements[k] = 255 - elements[k];
                		mIntermediateMat.put(i, j, elements);
                	}
                mIntermediateMat.convertTo(mIntermediateMat2, CvType.CV_8UC1);
                Imgproc.distanceTransform(mIntermediateMat2, mIntermediateMat, Imgproc.CV_DIST_C, 5);
                mIntermediateMat.convertTo(mIntermediateMat2, CvType.CV_8UC4);
                Imgproc.cvtColor(mIntermediateMat2, mRgba, Imgproc.COLOR_GRAY2RGBA);
            } break;
            case Sample2NativeCamera.VIEW_MODE_CANNY:
            {
                Imgproc.Canny(inputFrame, mIntermediateMat, 70, 100);
                Core.bitwise_not(mIntermediateMat, mIntermediateMat2);
                Imgproc.distanceTransform(mIntermediateMat2, mIntermediateMat, Imgproc.CV_DIST_C, 5);
                mIntermediateMat.convertTo(mIntermediateMat2, CvType.CV_8UC4);
                Imgproc.cvtColor(mIntermediateMat2, mRgba, Imgproc.COLOR_GRAY2RGBA);
            } break;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        mItemPreviewCanny = menu.add("Distance transform");
        return true;
    }
